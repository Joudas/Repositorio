...
const LoginPage = () => {
  ...
  const {loginData, setLoginData, submitLogin, resetLoginForm} = useContext(LoginContext);
  ...
  const handleSubmit = async (e) => {
    ...
    if(result.ok){
      ...
      setStatusValidate(["Error de Credenciales", true]);
    }
  };
          ...
          <div>
            <form className="grid gap-24 mt-16 text-base" onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-1 gap-4 h-10">
                {statusValidate[1] && <span className="text-sm text-red-500 w-full">{statusValidate[0]}</span>}
