...
export default function RegisterScreen() {
  ...
  // Extraer errores de validación del backend (Zod)
  const backendErrors = registerMutation.isError
    ? (() => {
        const err = registerMutation.error;
        if (err instanceof Error) {
          try {
            const parsed = JSON.parse(err.message);
            if (parsed?.details) return parsed.details as Array<{ field: string; message: string }>;
          } catch {
            // no es JSON, mostrar el mensaje directo
          }
        }
        return null;
      })()
  ...
  const fieldError = (field: string) =>
    backendErrors?.find((e) => e.field === field)?.message;
      ...
      <div className="rounded-lg lg:border-4 lg:border-white lg:bg-transparent lg:p-2 gap-2 flex lg:min-w-[65%] h-full sm:h-[70%] lg:h-[80%]">
        <div className="lg:w-[50%] rounded-lg w-screen h-full sm:w-120 flex bg-white">
          ...
          >
            ...
            <div className="h-12 -mt-4 mb-4">
              <div className="h-4">
                {registerMutation.isError && !backendErrors && (
            ...
            <div className="flex flex-col gap-4 mt-2 text-brand-muted">
              ...
              <div className="w-full text-center">
                <Link to="/login" className="text-brand-muted cursor-pointer lg:text-sm">
