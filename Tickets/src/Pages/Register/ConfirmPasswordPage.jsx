...
const ConfirmPasswordPage = () => {
  ...
  const { handleState, password, submitRegister, registerLoading, registerError, resetRegisterForm } = useContext(RegisterContext);
  ...
  const handleSubmit = async (e) => {
    ...
    if (password !== confirmPassword) {
      setConfirmMessage('Contraseña incorrecta');
      return;
    }

    const result = await submitRegister(confirmPassword);
          ...
          <div>
            <form className="grid gap-18 mt-16 text-base" onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 gap-2">
                <div className="relative">
                  <input
                    ...
                    placeholder="*****"
                  />
                  <span className="text-sm text-red-500">{confirmMessage || registerError}</span>
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    disabled={btnState || registerLoading}
              ...
              <div className="grid grid-cols-1 gap-4">
                <button type="submit" disabled={registerLoading} className="w-full h-10 btn-base cursor-pointer disabled:cursor-not-allowed disabled:opacity-70">
                  {registerLoading ? 'Registrando...' : 'Continuar'}
                </button>
              </div>
            </form>
          </div>
