...
export const TicketsList = ({priorityMap, statesMap, listTickets, actualTicket, defineTicket, loadingTickets, openModal}) => {
    const { resetForm } = useContext(TicketFormContext);
        ...
        <div className='col-span-4 h-full p-6 bg-white rounded-lg overflow-auto min-w-0'>
            ...
            <div className='mt-10 flex flex-col gap-10'>
                {loadingTickets ? (
                    ...
                    const priorityTicket = priorityMap[ticket?.priority] ?? { state: 'Baja', css : 'bg-green-500'}
                    const stateBanner = statesMap[ticket?.state] ?? { state: 'En Progreso', css : 'bg-yellow-500 hover:bg-yellow-600', border: 'ticket-border-yellow' };
                    return(
                    <div key={ticket.id} 
