...
export const TicketDetails = ({ priorityMap, statesMap, actualTicket, stateTicket, openModal }) => {
    const priority = actualTicket?.priority ?? '';
    const colorPriority = priorityMap[priority] ?? { label: '', color: 'bg-gray-400' };
    const colorState = statesMap[actualTicket?.state] ?? { label: '', border: 'border-yellow-500', text : 'text-yellow-500' };
