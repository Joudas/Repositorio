...
def stats_completed(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return CompletedStatsResponse(total_completed=int(stats.total_completed or 0), high_priority_completed=int(stats.high_priority_completed or 0))
...
def stats_in_progress(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return InProgressStatsResponse(total_in_progress=int(stats.total_in_progress or 0), high_priority_in_progress=int(stats.high_priority_in_progress or 0))
...
def stats_pending(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return PendingStatsResponse(total_pending=int(stats.total_pending or 0), high_priority_pending=int(stats.high_priority_pending or 0))
